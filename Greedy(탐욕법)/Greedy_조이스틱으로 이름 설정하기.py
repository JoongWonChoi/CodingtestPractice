...
def solution(name):
    ...
    #current i = index of 'A'
    print(i)
    left += (i-1)
    #0번째 인덱스까지 left 동작!
   ##'◀'(left) 동작을 수행하는 반복문(이름의 첫 번째 인덱스부터 시작합니다)
    while i+1<len(name):
        left += 1
        if name[i+1] in alp_front:
            up += alp_front.index(name[i+1])
        elif name[i+1] in alp_back:
            down += alp_back.index(name[i+1]) + 1
        i += 1
